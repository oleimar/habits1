...
// public:
    ...
    Gamete(gam_data_type&& g) : gam_dat{g} {}
    Gamete(const gam_data_type& g) : gam_dat{g} {}
    Gamete(unsigned n_loci = 0) : gam_dat(n_loci) {}
    const val_type& Value() const { return gam_dat; }
    flt& operator[](std::size_t i) { return gam_dat[i]; }
    flt operator[](std::size_t i) const { return gam_dat[i]; }
    std::size_t size() const { return gam_dat.size(); }
    void Mutate(MutRec& mr);
    // public data member
    gam_data_type gam_dat;
};

template<typename MutRec>
void  Gamete<MutRec>::Mutate(MutRec& mr)
{
    for (std::size_t i = 0; i < gam_dat.size(); ++i) {
        ...
        if (mr.mut_rate[i] > 0) {
            if (mr.uni(mr.eng) < mr.mut_rate[i]) {
                ...
                gam_dat[i] += mr.SD[i]*u;
                if (gam_dat[i] > mr.max_val[i]) gam_dat[i] = mr.max_val[i];
                else if (gam_dat[i] < mr.min_val[i]) gam_dat[i] = mr.min_val[i];
            }
        }
    }
}
...
Diplotype<GamType>::GetGamete(mut_rec_type& mr) const
{
    ...
    gam_data_type& gam_data = gam.gam_dat;
    const gam_data_type& mat_gam_data = mat_gam.gam_dat;
    const gam_data_type& pat_gam_data = pat_gam.gam_dat;
