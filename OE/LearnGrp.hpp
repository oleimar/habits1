...
public:
    ...
    flt w;        // estimated value of chosen stimulus dimension
    flt hs;       // consitency for chosen stimulus dimension
...
// stimuli, and perceives a reward. Next, the agent updates its state (method
// Update_agent in Phenotyep.hpp), after with there is potential mortality
// from predation.
...
void LrnGrp<PhenType>::Learn(rand_eng& eng)
{
    ...
    for (int j = 0; j < g; ++j) {
        ...
        for (int l = 0; l < nph; ++l) {
            ...
            for (int trial = 0; trial < T; ++trial) {
                if (agent.alive) {
                    // the current estimated value of vcs[k] is w[k]
                    vf_type smx(ncs, 0); // soft-max weights
                    flt w_max = *std::max_element(agent.w.begin(), 
                                                  agent.w.end());
                    for (int k = 0; k < ncs; ++k) {
                        // use max value as zero point (stable soft-max)
                        flt y = agent.beta*(agent.w[k] - w_max);
                        if (y < -50) { // extra safeguard
                            smx[k] = 0;
                        } else { // compute soft-max weights
                            smx[k] = std::exp(y);
                        }
                    }
                    rand_discr discr(smx.begin(), smx.end());
    ...
    // alternative 2: for each trial, run through all group members 
    // (not used, but suitable when there is frequency dependence)
}
