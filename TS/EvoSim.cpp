...
void Evo::ReplacePop()
{
    for (int grp = 0; grp < num_grps; ++grp) { // local groups
        for (int i = 0; i < G; ++i) { // individuals in group
            ...
            pop[j].genotype = gen;
            phen_type& ph = pop[j].phenotype;
            ph.Assign(gen, rho0, 1/(sd0*sd0), 1/(sdR*sdR), alph_hs);
...
void Evo::h5_read_pop(const std::string& infilename)
{
    ...
    read_and_assign_vf_type(h5, "y_bar", flt_pars, &phen_type::y_bar);
    read_and_assign_vf_type(h5, "rho_hat", flt_pars, &phen_type::rho_hat);
    read_and_assign_vf_type(h5, "tau_hat", flt_pars, &phen_type::tau_hat);
    read_and_assign_vf_type(h5, "att", flt_pars, &phen_type::att);
    read_and_assign_vf_type(h5, "hs", flt_pars, &phen_type::hs);
    // vi_type attributes
    vvi_type int_pars(N, vi_type(ncs));
    read_and_assign_vi_type(h5, "n_chs", int_pars, &phen_type::n_chs);
}
...
void Evo::h5_write_pop(const std::string& outfilename) const
{
    ...
    vvf_type flt_pars(N, vf_type(ncs));
    write_vf_type(h5, "y_bar", flt_pars, &phen_type::y_bar);
    write_vf_type(h5, "rho_hat", flt_pars, &phen_type::rho_hat);
    write_vf_type(h5, "tau_hat", flt_pars, &phen_type::tau_hat);
    write_vf_type(h5, "att", flt_pars, &phen_type::att);
    write_vf_type(h5, "hs", flt_pars, &phen_type::hs);
    // write vi_type phenotype attributes
    vvi_type int_pars(N, vi_type(ncs));
    write_vi_type(h5, "n_chs", int_pars, &phen_type::n_chs);
}
